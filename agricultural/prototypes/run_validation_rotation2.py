...
SOYBEAN = dict(tt_maturity=2250, flowering_tt=1250, base_t=5, opt_t=28, max_t=43,
               ...
               make_layers=make_layers, calibration_factor=0.9669,  # re-derived 2026-09-23 for the
               # real-data TTF50_SHOOT_PARTITION refit (0.5 -> 0.32) + the same-day emergence-gate
               # fix + the cold-temperature radiation-growth reduction (transpiration_temp_factor
               # now also applied to GR) -- see cycles_engine_validate.py
...
WHEAT = dict(tt_maturity=1800, flowering_tt=1250, base_t=0, opt_t=20, max_t=35,
             ...
             make_layers=make_layers, calibration_factor=0.6357,  # re-derived 2026-09-23 for the
             ...
             # (transpiration_temp_factor now also applied to GR, not just GT -- see the comment
             # above thermal_time_increment() -- this is the fix that improved wheat's own
             # correlation again, 0.276 -> 0.397, the largest single jump wheat has seen)
...
CORN_SILAGE = dict(tt_maturity=1800, flowering_tt=1000, base_t=6, opt_t=28, max_t=46,
                    ...
                    make_layers=make_layers, forage_fraction=0.95, calibration_factor=0.6744,  # re-derived
                    # 2026-09-23 for the real-data TTF50_SHOOT_PARTITION refit + the same-day
                    # emergence-gate fix + the cold-temperature radiation-growth reduction
                    # (transpiration_temp_factor now also applied to GR) -- see cycles_engine_validate.py
